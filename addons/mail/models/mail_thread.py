...
class MailThread(models.AbstractModel):
    ...
    def message_post(self, *,
        ...
        ############################################################
        # Send sticky notifications AFTER the regular ones
        # if partner_ids:
        ...
        #                 'message': tools.html2plaintext(body)[:250] if body else '',
        #                 'sticky': False,
        #                 'type': 'info',
        #             }
        #         )
        #         print(f"Sent sticky notification to: {partner.name}")
        ##############################################################
        return new_message
    ...
    def _notify_thread(self, message, msg_vals=False, **kwargs):
        ...
        if scheduled_date:
            ...
            # ADD STICKY NOTIFICATIONS FOR CHATTER MESSAGES ONLY:
            # Check if this is a chatter message (has message_type 'comment' and is not internal)
            # message_type = msg_vals.get('message_type') or (
            #     message.message_type if hasattr(message, 'message_type') else None)
            # is_internal = msg_vals.get('is_internal') or (
            #     message.is_internal if hasattr(message, 'is_internal') else False)
            #
            # # Show sticky notifications only for chatter messages (comments that are not internal)
            # if message_type == 'comment' and not is_internal:
            #     # Extract partner_ids from recipients_data
            #     partner_ids = [recipient_data['id'] for recipient_data in recipients_data if recipient_data.get('id')]
            #
            #     # Get sender's partner ID to exclude from sticky notifications
            #     sender_partner_id = None
            #     if hasattr(message, 'author_id') and message.author_id:
            #         sender_partner_id = message.author_id.id
            #     elif msg_vals.get('author_id'):
            #         sender_partner_id = msg_vals.get('author_id')
            #
            #     # Filter out sender from recipients for sticky notifications
            #     receiver_partner_ids = [pid for pid in partner_ids if pid != sender_partner_id]
            #
            #     # Send sticky notifications ONLY to receivers (not sender)
            #     if receiver_partner_ids:
            #         # Extract subject and body from message or msg_vals
            #         subject = msg_vals.get('subject') or (
            #             message.subject if hasattr(message, 'subject') else "New Chatter Message")
            #         body = msg_vals.get('body') or (message.body if hasattr(message, 'body') else '')
            #
            #         # Extract sender name
            #         sender_name = ""
            #         if hasattr(message, 'author_id') and message.author_id:
            ...
            #         elif msg_vals.get('author_id'):
            #             author = self.env['res.partner'].browse(msg_vals.get('author_id'))
            #             sender_name = author.name if author else ""
            #
            #         # Format the message with sender name
            #         message_text = tools.html2plaintext(body)[:250] if body else ''
            #         if sender_name:
            #             formatted_message = f"{sender_name}: {message_text}"
            #         else:
            #             formatted_message = message_text
            #
            #         # Get the record name for context
            #         record_name = self.display_name if hasattr(self, 'display_name') else ""
            #         if record_name and subject:
            #             notification_title = f"{subject} - {record_name}"
            #         elif record_name:
            #             notification_title = f"New message on {record_name}"
            #         else:
            #             notification_title = subject or "New Chatter Message"
            #
            #         receivers = self.env['res.partner'].browse(receiver_partner_ids)
            ...
            #                 }
            #             )
            #             print(f"Sent sticky chatter notification to receiver: {receiver.name} from: {sender_name}")
