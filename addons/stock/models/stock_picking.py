...
class Picking(models.Model):
    ...
    def _compute_products_availability(self):
        pickings = self.filtered(lambda picking: picking.state in (
            'waiting', 'confirmed', 'assigned') and picking.picking_type_code == 'outgoing')
    ...
    def _check_move_lines_map_quant_package(self, package):
        return package._check_move_lines_map_quant(
            self.move_line_ids.filtered(lambda ml: ml.package_id == package and ml.product_id.type == 'product'))
        ...
        for package in self.move_line_ids.package_id:
            ...
            if pickings._check_move_lines_map_quant_package(package):
                ...
                move_lines_to_pack = pickings.move_line_ids.filtered(
                    lambda ml: ml.package_id == package and not ml.result_package_id and ml.state not in (
                        'done', 'cancel'))
