...
Please change the quantity done or the rounding precision of your unit of measure.""",
    ...
    @api.depends('has_tracking', 'picking_type_id.use_create_lots', 'picking_type_id.use_existing_lots', 'state',
                 'origin_returned_move_id', 'product_id.detailed_type', 'picking_code')
    ...
    def _check_uom(self):
        ...
        if moves_error:
            user_warnings = [
                ...
                *(
                    _('%s --> Product UoM is %s (%s) - Move UoM is %s (%s)', move.product_id.display_name,
                      move.product_id.uom_id.name, move.product_id.uom_id.category_id.name, move.product_uom.name,
    ...
    def action_generate_lot_line_vals(self, context, mode, first_lot, count, lot_text):
        ...
        if default_vals.get('picking_type_id'):
            ...
            if picking_type.use_existing_lots:
                self._create_lot_ids_from_move_line_vals(vals_list, default_vals['product_id'],
    ...
    def _push_apply(self):
        ...
        for move in self:
            ...
            if move.location_dest_id.company_id == self.env.company:
                rule = self.env['procurement.group']._search_rule(move.route_ids, move.product_packaging_id,
                ...
                rule = procurement_group._search_rule(move.route_ids, move.product_packaging_id, move.product_id, False,
                                                      domain)
    ...
    def _onchange_lot_ids(self):
        ...
        if quants:
            sn_to_location = ""
            for quant in quants:
                sn_to_location += _("\n(%s) exists in location %s", quant.lot_id.display_name,
                                    quant.location_id.display_name)
            return {
                'warning': {'title': _('Warning'), 'message': _(
                    'Existing Serial numbers. Please correct the serial numbers encoded:') + sn_to_location}
            }
    ...
    def _check_unlink_move_dest(self):
        ...
        for move in self:
            move_dest_ids_to_unlink = move.move_dest_ids.filtered(
                lambda m: move.location_dest_id != m.location_id and str(
                    m.location_id.id) not in move.location_dest_id.parent_path.split('/')
            )
    ...
    def _adjust_procure_method(self):
        ...
        for move in self:
            ...
            rules = self.env['procurement.group']._search_rule(False, move.product_packaging_id, product_id,
