...
//        this.searchReadEmployee();
        ...
        // 👇 Lightweight init fetch, avoids inherited searchReadEmployee
        this.initAttendanceState();
    }

    async initAttendanceState() {
        try {
            ...
            this.state.isDisplayed = result.display_systray;
        } catch (err) {
            console.warn("⚠️ Failed to fetch attendance state:", err);
            this.state.checkedIn = false; // fallback red
        }
    }
