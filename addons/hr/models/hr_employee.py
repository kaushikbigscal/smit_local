...
class HrEmployeePrivate(models.Model):
    ...
    create_user = fields.Boolean(string="Auto Create User", default=False)
    disable_tracking = fields.Boolean(string = "Disable Tracking", default = False)
    ...
    def load(self, fields, data):
        """
        Override load to handle department/job creation in correct company
        ONLY during employee imports from CSV/Excel files
        """
        # Very specific check: Only for employee imports with dept/job fields
        is_employee_import = (
                                     'department_id' in fields or 'job_id' in fields  # Has dept/job fields
                             ) and (
                                     len(data) > 1 or  # Multiple records (typical import)
                                     any(isinstance(row[i], str) for row in data for i in range(len(row)) if row[i])
                                 # Has string values (import scenario)
                             )

        # Only apply during employee imports
        if is_employee_import:
            processed_data = []

            company_idx = fields.index('company_id') if 'company_id' in fields else None
            dept_idx = fields.index('department_id') if 'department_id' in fields else None
            job_idx = fields.index('job_id') if 'job_id' in fields else None

            for row in data:
                processed_row = list(row)

                # Handle department
                if company_idx is not None and dept_idx is not None:
                    company_name = row[company_idx]
                    dept_name = row[dept_idx]

                    if company_name and dept_name and isinstance(dept_name, str):
                        ...
                        if company:
                            ...
                            if not dept:
                                dept = self.env["hr.department"].sudo().with_company(company.id).create({
                                    "name": dept_name,
                                    "company_id": company.id,
                                })
                            processed_row[dept_idx] = dept.name

                # Handle job
                if company_idx is not None and job_idx is not None:
                    company_name = row[company_idx]
                    job_name = row[job_idx]

                    if company_name and job_name and isinstance(job_name, str):
                        company = self.env["res.company"].search([("name", "=", company_name)], limit=1)
                        if company:
                            ...
                            if not job:
                                job = self.env["hr.job"].sudo().with_company(company.id).create({
                                    "name": job_name,
                                    "company_id": company.id,
                                })
                            processed_row[job_idx] = job.name

                processed_data.append(processed_row)

            return super().load(fields, processed_data)
    ...
    #         Override load to handle department/job creation in correct company
    #         """
    #         # Pre-process data to create departments/jobs in correct companies
    #         processed_data = []
    #
    #         # Find company_id, department_id, job_id column indices
    #         company_idx = fields.index('company_id') if 'company_id' in fields else None
    #         dept_idx = fields.index('department_id') if 'department_id' in fields else None
    #         job_idx = fields.index('job_id') if 'job_id' in fields else None
    #
    ...
    #
    #                 if company_name and dept_name and isinstance(dept_name, str):
    #                     # Find or create department in correct company
    #                     company = self.env["res.company"].search([("name", "=", company_name)], limit=1)
    #                     if company:
    #                         dept = self.env["hr.department"].search([
    #                             ("name", "=", dept_name),
