...
class AccountAnalyticLine(models.Model):
    ...
    def default_get(self, field_list):
        ...
        if not self.env.context.get('default_employee_id') and 'employee_id' in field_list and result.get('user_id'):
            ...
            'company_id', '=', result.get('company_id', self.env.company.id))], limit=1).id
    ...
    def _check_can_write(self, values):
        ...
        if not (self.user_has_groups('hr_timesheet.group_hr_timesheet_approver') or self.env.su) and any(
    ...
    def _apply_time_label(self, view_node, related_model):
        ...
        for node in doc.xpath(
            ...
            name_with_uom = re.sub(re.escape(_('Hours')) + "|Hours", encoding_uom_name or '',
                                   Model._fields[node.get('name')]._description_string(self.env), flags=re.IGNORECASE)
    ...
    def _timesheet_preprocess(self, vals_list):
        ...
        for index in timesheet_indices:
            ...
            if not vals.get('account_id'):
                ...
                if not account or not account.active:
                    raise ValidationError(
                        _('Timesheets must be created on a project or a task with an active analytic account.'))
            ...
            if not vals.get('product_uom_id'):
                ...
                vals['product_uom_id'] = uom_id_per_company.get(company.id,
                                                                company.project_time_mode_id.id) or self.env.company.project_time_mode_id.id
