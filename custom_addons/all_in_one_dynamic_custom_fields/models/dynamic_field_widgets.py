...
class DynamicFieldWidgets(models.Model):
    ...
    @api.model
    def update_binary_to_many2many_binary(self):
        record = self.env.ref('all_in_one_dynamic_custom_fields.dynamic_field_widget_binary', raise_if_not_found=False)
        if record and record.name == 'binary':
            record.name = 'many2many_binary'
