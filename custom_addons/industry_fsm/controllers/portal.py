...
class CustomerPortal(portal.CustomerPortal):
    ...
    @route(['/my/task/<int:task_id>/report',
            '/my/task/<int:task_id>/report/<string:source>',
            '/my/task/<int:task_id>/report/sign/<string:source>'], type='http', auth="public")
    def portal_worksheet_outdated(self, **kwargs):
        return request.redirect(request.httprequest.path.replace('/my/task/', '/my/tasks/'))

    @route(['/my/tasks/<int:task_id>/report/sign/<string:source>'], type='json', auth="public", website=True)
    def portal_worksheet_sign(self, task_id, access_token=None, source=False, name=None, signature=None):
        ...
        try:
            task_sudo.write({
                ...
                'report_signed_by': name,
            })
        except (TypeError, binascii.Error):
            return {'error': _('Invalid signature data.')}

        pdf = request.env['ir.actions.report'].sudo()._render_qweb_pdf('industry_fsm_sale.action_custom_report_field_service',
                                                                       [task_sudo.id])[0]
    ...
    def _show_task_report(self, task_sudo, report_type, download):
        ...
        return self._show_report(model=task_sudo, report_type=report_type,
                                 report_ref='industry_fsm_sale.action_custom_report_field_service', download=download)
