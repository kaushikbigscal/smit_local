...
class Project(models.Model):
    ...
    # is_fsm = fields.Boolean("Field Service", default=False,
    #                         help="Display tasks in the Field Service module and allow planning with start/end dates.")
    ...
    def _compute_display_name(self):
        ...
        if len(self.env.context.get('allowed_company_ids', [])) <= 1:
            return
        fsm_project_default_name = _("Service Call")
        for project in self:
    ...
    _sql_constraints = [
        ('company_id_required_for_fsm_project', "CHECK((is_fsm = 't' AND company_id IS NOT NULL) OR is_fsm = 'f')",
    ...
    def default_get(self, fields_list):
        ...
        if 'allow_task_dependencies' in fields_list:
            defaults['allow_task_dependencies'] = defaults.get('allow_task_dependencies', False) and not defaults.get(
                'is_fsm')
