...
class CrmSalespersonPlannerVisitTemplate(models.Model):
    ...
    tour_plan_type = fields.Selection(
        selection=[
            ('city_wise','City-wise'),
    ...
    zip_code= fields.Many2one('city.zip', string='Zip Code' , domain="[('city_id', '=?', city_id)]")
    partner_ids = fields.Many2many(
        comodel_name="res.partner",
        string="Customer",
        default=False,
        required=True,
        domain=[('zip', '=', zip_code.name)]
    ...
    #    print(f"Selected zip_code: {self.zip_code.name}")
    ...
    def _onchange_zip_code(self):
        ...
        if self.zip_code:
            ...
            partners = self.env['res.partner'].search([('zip', '=', self.zip_code.name)])
    ...
    def action_validate(self):
        if not self.partner_ids:
            raise ValidationError(_("Please select a customer (Partner) before validating."))
    ...
    def _prepare_crm_salesperson_planner_visit_vals(self, dates):
        ...
        for date in dates:
            for partner in self.partner_ids:
                visit_vals.append({
                    "partner_id": partner.id,  # Use the current partner ID
                    "date": date,
