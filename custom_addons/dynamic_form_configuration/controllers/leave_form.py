...
class LeaveForm(http.Controller):
    ...
    @http.route('/web/hr_leave_form', type='json', auth='user')
    def get_leave_form(self):
        return self._get_mobile_form_view('hr.leave', 'hr_holidays.hr_leave_view_form')
    ...
    def _get_mobile_form_view(self, model_name, view_xml_id):
        try:
            ...
            # Step 4: Dynamic fields (optional, if applicable for hr.leave)
            dynamic_fields = request.env['dynamic.fields'].search([
                ('model', '=', model_name),
                ('form_view_id.name', '=', 'hr.leave.view.form'),
                ('status', '=', 'form')
            ])
