...
class ActivityScheduleFormController(http.Controller):

    @http.route('/web/activity_schedule_form', type='json', auth='user')
    def get_activity_form(self):
        return self._get_activity_schedule_mobile_form_view('mail.activity.schedule',
                                                            'mail.mail_activity_schedule_view_form')

    def _get_activity_schedule_mobile_form_view(self, model_name, view_xml_id):
        try:
            ...
            if form_elements:
                ...
                sheet_fields = list(form_element.iter("field"))
            else:
                sheet_fields = []
            ...
            # Step 4: Add dynamic fields (if any exist)
            dynamic_fields = request.env['dynamic.fields'].search(
                [('model', '=', model_name), ('status', '=', 'form')]
            )
