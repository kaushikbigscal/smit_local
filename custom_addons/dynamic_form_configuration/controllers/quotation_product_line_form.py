...
class SaleOrderLineForm(http.Controller):

    @http.route('/web/sale_order_line_form', type='json', auth='user')
    def get_sale_order_line_form(self):
        return self._get_mobile_form_view('sale.order.line', 'sale.view_order_form')
    ...
    def _get_mobile_form_view(self, model_name, view_xml_id):
        try:
            ...
            # Find tree view under field name 'order_line'
            sheet_fields = []
            for field in arch_tree.iter("field"):
                if field.get("name") == "order_line":
                    for tree in field.iter("tree"):
                        sheet_fields += list(tree.iter("field"))
            ...
            dynamic_fields = request.env['dynamic.fields'].search([
                ('model', '=', model_name),
                ('form_view_id.name', '=', 'sale.order.line.form.readonly'),
                ('status', '=', 'form')
            ])
